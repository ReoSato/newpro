...
dl_line(280,430,0,135,DL_C("white"), 4);//ライン//
dl_line(360,430,640,135,DL_C("white"), 4);
//マウンドプレート//

dl_text(SCORE,400,440,1.0,DL_C("white"),4); //スコア&アウト 文字表示//
dl_text(OUT,40,440,1.0,DL_C("white"),4);
 if (out == 1) {
 dl_circle(130, 430,12, DL_C("red"),1,1);//アウトカウント描画//
 }
 if (out == 2) {
 dl_circle(130, 430,12, DL_C("red"),1,1);
 dl_circle(160, 430,12, DL_C("red"),1,1);
  }
dl_line(280,415,280 + r*cos(start),415 - r*sin(start),DL_RGB(145,150,155), 5);//バット描画//
 if (bally > 510 || bally < -17 || ballx > 700 || ballx < -15) {//初期化//
 ballx = 320; bally = 10;
...
dl_line(240,40,300,40,DL_C("black"), 7);
dl_line(300,2,340,2,DL_C("white"), 7);
dl_line(340,40,400,40,DL_C("black"), 7);
